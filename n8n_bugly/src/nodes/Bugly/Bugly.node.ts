...
function uuidV4(): string {
  ...
  return `${hex[b[0]]}${hex[b[1]]}${hex[b[2]]}${hex[b[3]]}-${hex[b[4]]}${
      hex[b[5]]}-${hex[b[6]]}${hex[b[7]]}-${hex[b[8]]}${hex[b[9]]}-${
      hex[b[10]]}${hex[b[11]]}${hex[b[12]]}${hex[b[13]]}${hex[b[14]]}${
      hex[b[15]]}`;
}

function parseBuglyCrashUrl(crashUrl: string):
    {appId: string; issueId: string; pid: string} {
  const u = new URL(crashUrl.trim());
  const m = u.pathname.match(
      /\/v2\/crash-reporting\/(?:crashes|errors)\/([^/]+)\/([^/]+)/);
...
async function requestAny({helpers, Referer, url, method, json}: {
  ...
  const credentials = await helpers.getCredentials('buglyApi') as
...
async function getCrashDetailByCrashUrl({helpers, i, crashUrl}: {
  helpers: IExecuteFunctions; i: number; crashUrl: string
}) {
  const debug = helpers.getNodeParameter('debug', 0) as boolean;
  const baseUrl = 'https://bugly.qq.com';
  const {appId, issueId, pid} = parseBuglyCrashUrl(crashUrl);
  if (debug) console.log('appId', appId, 'issueId', issueId, 'pid', pid);
  let fsn = uuidV4();
  ...
  const last_res =
      await requestAny({helpers, Referer: crashUrl, url, method: 'GET'});
...
async function getIssueListByManualSearch(
    {helpers, i}: {helpers: IExecuteFunctions; i: number;}) {
  const baseUrl = 'https://bugly.qq.com';
  ...
  const packageVersion =
      helpers.getNodeParameter('packageVersion', i) as string;
  ...
  for (let guard = 0; guard < 200; guard++) {
    ...
    for (const issue of issueList) {
      const crashUrl =
          `${baseUrl.replace(/\/+$/, '')}/v2/crash-reporting/crashes/${
              appIdClean}/${issue.issueId}?pid=${pidClean}`;
...
async function getuserId({helpers, baseUrl}: {
  helpers: IExecuteFunctions; baseUrl: string;
}): Promise<string> {
  try {
    const debug = helpers.getNodeParameter('debug', 0) as boolean;
...
async function getAppList({helpers, baseUrl, userId}: {
  helpers: IExecuteFunctions; baseUrl: string; userId: string;
}): Promise<any[]> {
  try {
    const url = `${baseUrl.replace(/\/+$/, '')}/v4/api/old/app-list?userId=${
        encodeURIComponent(userId)}&fsn=${uuidV4()}`;
    const resp = await requestAny({helpers, Referer: url, url, method: 'GET'});
    if (resp.error) throw resp.error;
    return resp.data;
  } catch (error) {
      ...
      async getBuglyAppList(
        ...
        return list
            .map((a: any) => {
              ...
              const name = String(
                               a?.appName ?? a?.name ?? a?.productName ??
    ...
    for (let i = 0; i < items.length; i++) {
      if (this.getNodeParameter('mode', i) === 'manual') {
        ...
        for (const crashUrl of crashUrls) {
          try {
            const detail =
                await getCrashDetailByCrashUrl({helpers: this, i: i, crashUrl});
