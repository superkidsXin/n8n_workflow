...
function buildSoNameMapJsonFromEntries(entries: any[]): string {
    const root: any = {};
    for (const row of entries || []) {
        ...
        if (!pkg || !soName) continue;
        if (!root[pkg]) root[pkg] = {};
        if (!root[pkg][ver]) root[pkg][ver] = {};
        root[pkg][ver][soName] = {};
        if (arm32) root[pkg][ver][soName].arm32 = arm32;
        if (arm64) root[pkg][ver][soName].arm64 = arm64;
    }
    return JSON.stringify(root);
}
function pickSoNameMapByPackageAndVersion(text: string, packageName: string, appversion: string): SoNameMap {
    const raw = String(text || '').trim();
    if (!raw) throw new Error('so_name_map is empty');
    ...
    if (pkg) {
        if (!container || typeof container !== 'object' || Array.isArray(container)) throw new Error(`so_name_map missing packageName: ${pkg}`);
    }
    const ver = String(appversion || '').trim();
    const selected = ver ? container?.[ver] : (container?.default ?? container?.latest ?? container);
    if (ver) {
        if (!selected || typeof selected !== 'object' || Array.isArray(selected)) throw new Error(`so_name_map missing version: ${ver}${pkg ? ` packageName: ${pkg}` : ''}`);
    }
    return selected;
}
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                const pakageName = this.getNodeParameter('pakageName', i) as string;
                const soNameMapText = buildSoNameMapJsonFromEntries(((this.getNodeParameter('soNameMapForm', i) as any)?.rows as any[]) || []);
